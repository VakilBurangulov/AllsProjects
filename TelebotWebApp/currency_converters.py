...
bot = telebot.TeleBot('7327305435:AAEfH609dp0g7Kgbmp7ADQpp7J_OHr8NEbU')
...
def callback(call):
    if call.data != 'else':
        ...
        bot.send_message(call.message.chat.id, f'{amount} {values[0]} = {round(res, 2)} {values[1]}. Можете заново вписать сумму')
...
def my_currency(message):
    try:
        ...
        bot.send_message(message.chat.id,
                         f'{amount} {values[0]} = {round(res, 2)} {values[1]}. Можете заново вписать сумму')
        bot.register_next_step_handler(message, summa)
